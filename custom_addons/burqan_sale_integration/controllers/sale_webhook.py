...
from odoo.addons.burqan_sale_integration.models.sale_order import BurqanWebhookError
...
class BurqanSaleWebhook(http.Controller):
    ...
    def sale_completed(self, **kwargs):
        ...
        try:
            ...
            _logger.info(
                'Burqan webhook %s orderId=%s sale.order=%s',
                'reused' if reused else 'created',
                order_id,
                order.id,
            )
            return request.make_json_response(
                {'ok': True, 'saleOrderId': order.id},
                status=200,
            )
